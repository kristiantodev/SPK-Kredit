...
class kelayakan extends CI_Controller {
	...
	public function index()
	{
        ...
        $this->load->view('bagiankredit/layak',$data);
    ...
    public function add()
    {
        ...
        if($this->form_validation->run()==FALSE){
            ...
            redirect('bagiankredit/kelayakan');
        }else{

            $i=1;
            while (isset($_POST['id_kredit'.$i])) {

            $data=array(
                "vektor"=> $_POST['vektor'.$i],
                "keputusan"=> $_POST['keputusan'.$i]
            );

            $this->db->where('id_kredit', $_POST['id_kredit'.$i] );
